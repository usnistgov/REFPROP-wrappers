    ...
    # If the provided string is a path, then we use it to find any shared libraries
        ...
        if len(sos) == 0:
            ...
            for so in sos:
                ...
                p = Path(so)
                # Skip symbolic links
                # the two dll generated by REFPROP-cmake have one symlinking to the other one,
                # so help the user by skipping the one that is a symlink
                if p.is_symlink() and p.exists():
                    warnings.warn(f'symbolic link shared library has been skipped: {so}')
                    continue
