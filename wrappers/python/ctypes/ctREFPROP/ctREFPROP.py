...
on 19 Jan 2018 13:40:06.  This interface was written by Ian Bell. 
...
class REFPROPFunctionLibrary():
    ...
    def __init__(self, name, shared_extension = None):
        ...
        self._ALLPROP1dlloutput_tuple = namedtuple('ALLPROP1dlloutput',["c","ierr","herr"])
        self._ALLPROPSdlloutput_tuple = namedtuple('ALLPROPSdlloutput',["Output","hUnits","iUCodeArray","ierr","herr"])
        self._ALLPRP200dlloutput_tuple = namedtuple('ALLPRP200dlloutput',["Output","hUnits","iUCodeArray","ierr","herr"])
        ...
        self._REFPROPdlloutput_tuple = namedtuple('REFPROPdlloutput',["zm","Output","hUnits","iUCode","x","y","x3","q","ierr","herr"])
        ...
        self._ALLPROP1dll = self._getfcn(self.dll, 'ALLPROP1dll')
    ...
    def ALLPROP1dll(self,hOut,iUnits,T,D,z):
        """
        c,ierr,herr = ALLPROP1dll(hOut,iUnits,T,D,z)
        """
        ...
        if self._ALLPROP1dll is None: raise ValueError("The function ALLPROP1dll could not be loaded from the shared library.") 
        self._ALLPROP1dll(hOut,ct.byref(iUnits),ct.byref(T),ct.byref(D),z,ct.byref(c),ct.byref(ierr),herr,255,255)

        return self._ALLPROP1dlloutput_tuple(c.value,ierr.value,trim(herr.raw))

    def ALLPROPSdll(self,hOut,iUnits,iMass,iFlag,T,D,zm):
        """
        Output,hUnits,iUCodeArray,ierr,herr = ALLPROPSdll(hOut,iUnits,iMass,iFlag,T,D,zm)
        """
        hOut = ct.create_string_buffer(hOut.encode('utf-8'),255)
        iUnits = ct.c_long(iUnits)
        ...
        self._ALLPROPSdll(hOut,ct.byref(iUnits),ct.byref(iMass),ct.byref(iFlag),ct.byref(T),ct.byref(D),zm,Output,hUnits,iUCodeArray,ct.byref(ierr),herr,255,1000,255)

        return self._ALLPROPSdlloutput_tuple(list(Output),trim(hUnits.raw),list(iUCodeArray),ierr.value,trim(herr.raw))

    def ALLPRP200dll(self,hOut,iUnits,iMass,iFlag,T,D,zm):
        """
        Output,hUnits,iUCodeArray,ierr,herr = ALLPRP200dll(hOut,iUnits,iMass,iFlag,T,D,zm)
        """
        ...
        self._ALLPRP200dll(hOut,ct.byref(iUnits),ct.byref(iMass),ct.byref(iFlag),ct.byref(T),ct.byref(D),zm,ct.byref(Output),hUnits,iUCodeArray,ct.byref(ierr),herr,10000,10000,255)

        return self._ALLPRP200dlloutput_tuple(Output.value,trim(hUnits.raw),list(iUCodeArray),ierr.value,trim(herr.raw))
    ...
    def REFPROP1dll(self,hIn,hOut,iUnits,a,b,z):
        """
        z,c,q,ierr,herr = REFPROP1dll(hIn,hOut,iUnits,a,b,z)
        """
        ...
        self._REFPROP1dll(hIn,hOut,ct.byref(iUnits),ct.byref(a),ct.byref(b),z,ct.byref(c),ct.byref(q),ct.byref(ierr),herr,255,255,255)

        return self._REFPROP1dlloutput_tuple(list(z),c.value,q.value,ierr.value,trim(herr.raw))

    def REFPROPdll(self,hFld,hIn,hOut,iUnits,iMass,iFlag,a,b,zm):
        """
        zm,Output,hUnits,iUCode,x,y,x3,q,ierr,herr = REFPROPdll(hFld,hIn,hOut,iUnits,iMass,iFlag,a,b,zm)
        """
        ...
        self._REFPROPdll(hFld,hIn,hOut,ct.byref(iUnits),ct.byref(iMass),ct.byref(iFlag),ct.byref(a),ct.byref(b),zm,Output,hUnits,ct.byref(iUCode),x,y,x3,ct.byref(q),ct.byref(ierr),herr,10000,255,255,255,255)

        return self._REFPROPdlloutput_tuple(list(zm),list(Output),trim(hUnits.raw),iUCode.value,list(x),list(y),list(x3),q.value,ierr.value,trim(herr.raw))
