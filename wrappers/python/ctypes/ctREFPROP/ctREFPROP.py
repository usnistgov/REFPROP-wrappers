...
on 14 Nov 2017 17:14:02.  This interface was written by Ian Bell. 
...
class REFPROPFunctionLibrary():
    ...
    def __init__(self, name, shared_extension = None):
        ...
        self._ALLPROPSdlloutput_tuple = namedtuple('ALLPROPSdlloutput',["Output","hUnits","iUnits","ierr","herr"])
        self._ALLPRP200dlloutput_tuple = namedtuple('ALLPRP200dlloutput',["Output","hUnits","iUnits","ierr","herr"])
        ...
        self._REFPROPdlloutput_tuple = namedtuple('REFPROPdlloutput',["Output","hUnits","iUnits","x","y","x3","q","ierr","herr"])
    ...
    def ALLPROPSdll(self,hOut,iUnitNumb,iMass,iFlag,T,D,zm):
        """
        Output,hUnits,iUnits,ierr,herr = ALLPROPSdll(hOut,iUnitNumb,iMass,iFlag,T,D,zm)
        """
        hOut = ct.create_string_buffer(hOut.encode('utf-8'),255)
        iUnitNumb = ct.c_long(iUnitNumb)
        ...
        self._ALLPROPSdll(hOut,ct.byref(iUnitNumb),ct.byref(iMass),ct.byref(iFlag),ct.byref(T),ct.byref(D),zm,Output,hUnits,iUnits,ct.byref(ierr),herr,255,1000,255)

        return self._ALLPROPSdlloutput_tuple(list(Output),trim(hUnits.raw),list(iUnits),ierr.value,trim(herr.raw))

    def ALLPRP200dll(self,hOut,iUnitNumb,iMass,iFlag,T,D,zm):
        """
        Output,hUnits,iUnits,ierr,herr = ALLPRP200dll(hOut,iUnitNumb,iMass,iFlag,T,D,zm)
        """
        ...
        iUnits = (200*ct.c_long)()
        ierr = ct.c_long()
        herr = ct.create_string_buffer(255)

        if self._ALLPRP200dll is None: raise ValueError("The function ALLPRP200dll could not be loaded from the shared library.") 
        self._ALLPRP200dll(hOut,ct.byref(iUnitNumb),ct.byref(iMass),ct.byref(iFlag),ct.byref(T),ct.byref(D),zm,ct.byref(Output),hUnits,iUnits,ct.byref(ierr),herr,10000,10000,255)

        return self._ALLPRP200dlloutput_tuple(Output.value,trim(hUnits.raw),list(iUnits),ierr.value,trim(herr.raw))
    ...
    def REFPROPdll(self,hFld,hIn,hOut,iUnitNumb,iMass,iFlag,a,b,zm):
        """
        Output,hUnits,iUnits,x,y,x3,q,ierr,herr = REFPROPdll(hFld,hIn,hOut,iUnitNumb,iMass,iFlag,a,b,zm)
        """
        ...
        self._REFPROPdll(hFld,hIn,hOut,ct.byref(iUnitNumb),ct.byref(iMass),ct.byref(iFlag),ct.byref(a),ct.byref(b),zm,Output,hUnits,ct.byref(iUnits),x,y,x3,ct.byref(q),ct.byref(ierr),herr,10000,255,255,255,255)

        return self._REFPROPdlloutput_tuple(list(Output),trim(hUnits.raw),iUnits.value,list(x),list(y),list(x3),q.value,ierr.value,trim(herr.raw))
